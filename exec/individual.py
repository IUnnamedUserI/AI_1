...
cities = {
    'Лонгфорд': {'Ньюнхем': 31.4, 'Экстон': 37.1, 'Бреона': 51.4,
                 'Конара': 43.2, 'Дерби': 111.9},
...
def find_routes(cities, start, end):
    routes = []

    def dfs(path, current_city):
        if current_city == end:
            routes.append(path)
            return
...
# Основной код
all_routes = find_routes(symmetric_cities, start_city, end_city)
...
print(f"Все маршруты из {start_city} в {end_city} :")
for route, distance in sorted_routes:
    print(f"Маршрут: {' -> '.join(route)}, "
          f"Расстояние: {round(distance, 1)} км")
